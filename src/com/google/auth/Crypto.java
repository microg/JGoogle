...
public class Crypto {
	...
	private static PublicKey createKey(final String keyStr,
		...
		try {
			...
			final MessageDigest sha1 = MessageDigest
					.getInstance(CRYPT_HASH_TYPE);
			final byte hash[] = sha1.digest(data);
			ciphertextHeader[0] = 0;
			System.arraycopy(hash, 0, ciphertextHeader, 1, 4);

			return KeyFactory.getInstance(CRYPT_TYPE).generatePublic(
	...
	public static String encryptPassword(final String email,
			final String password) {
		return encryptPassword(email, password, CRYPT_PUBLIC_KEY);
	}

	private static String encryptPassword(final String email,
			final String password, final String publicKeyData) {
		final String combined = email + CRYPT_DIVIDER + password;
		return encryptString(combined, publicKeyData);
	}

	private static String encryptString(final String plain,
		...
		try {
			...
			for (int i = 0; i < chunks; i++) {
				...
				final byte[] cipherbytes = cipher.doFinal(plainbytes, start,
						length);
				System.arraycopy(ciphertextHeader, 0, bytes, i * 133,
						ciphertextHeader.length);
				System.arraycopy(cipherbytes, 0, bytes, i * 133
						+ ciphertextHeader.length, cipherbytes.length);
			}
